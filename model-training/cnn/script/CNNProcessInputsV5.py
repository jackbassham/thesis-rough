...
PATH_SOURCE = "/home/jbassham/jack/data/sh/inputs_v5"
...
def main():
    ...
    # Convert NaN values in uncertainty to average uncertainty
    rt_filt = np.where(np.isnan(rt), 1e3, rt)

    # NOTE - also try setting just land to 1e3, 
    # while setting other nans (no ice, no ice seasonal) to mean(r)

    print("Uncertainty NaNs Converted to 2000")
