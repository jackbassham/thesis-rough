...
# 3. Normalized uncertainty by 10 cm/s instead of cit_std (~7 cm/s)
######################################################################

START_YEAR = 1992
END_YEAR = 2020
HEM = 'sh'
VERSION = 'weightedtorchV2'

PATH_SOURCE = "/home/jbassham/jack/data/sh/inputs_v3/cnn_inputs"
PATH_DEST = "/home/jbassham/jack/data/sh/outputs_v3"
...
# def WeightedNRMSEloss(pred, true, uncertainty, eps=1e-4):
    
#     uncertainty_clamped = torch.clamp(uncertainty, min = eps)
    
#     se = (pred - true) ** 2                     # Square error [B, C, H, W]

#     weights = 1.0 / uncertainty_clamped     # Weights (divide by uncertainty) [B, 1, H, W]

#     # Muliply square error by weights (batchwise)
#     weighted_se = se * weights                  # [B, C, H, W]

#     # Take mean for mse
#     mse = torch.mean(weighted_se)

#     # Take standard deviation 
#     std = torch.std(true, unbiased = False) + eps
#     return torch.sqrt(mse) / std

# # NOTE 
# # 1. Change weights from 1/r to 1/(r**2)
